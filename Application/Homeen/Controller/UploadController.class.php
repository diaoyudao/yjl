...
class UploadController extends Controller
{
        ...
        if (!$info) {
            ...
            if (is_file(C('IMAGE_SAVE_PATH_ROOT') . $image_url)) {
                if ('' != (I('get.thb', ''))) {  // 需要生成缩略图
                    ...
                    // 创建缩略图
                    \Adminen\Common\Image::createPreviewImage(C('IMAGE_SAVE_PATH_ROOT') . $image_url,
    ...
    {
        // (new \Adminen\Common\Upload\Image\Preview())->show();
    }

    public function honestfile()
    {
        $upload = new \Adminen\Common\Upload\File();// 实例化上传类
        $upload->maxSize = 2*1024*1024 ;// 设置附件上传大小
        $upload->exts = array('xls','xlsx','cvs','jpg','png','gif','rar','zip');// 设置附件上传类型
        $upload->rootPath = C('UPLOAD_FILE_ROOT_PATH'); // 设置附件上传根目录
        $upload->savePath = C('UPLOAD_FILE_SAVE_PATH'); // 设置附件上传（子）目录
        $upload->saveName = 'md5';
        $upload->subName = '/';       // 禁止自动添加子目录
        $upload->saveExt = 'self_ext';    // 文件名后缀
        ...
        if (!$info) {
            ...
            $file_url = $info['file']['savepath'] . $info['file']['savename'];

            $this->ajaxReturn(['status' => 'ok', 'file_url' => $file_url, 'file_name' => $info['file']['name']]);
        }
    }
}