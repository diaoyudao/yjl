...
<block name="body_main">
    <article class="page-container">
        <form class="form form-horizontal" id="form-advert-add" enctype="multipart/form-data">
            <input type="hidden"
                   value="<empty name='Think.get.land_id'>{$image.land_id}<else />{$Think.get.land_id}</empty>"
                   name="land_id"/>
            ...
            <div class="row cl">
                ...
                <select name="tags" class="select">
                    ...
                    <volist name="lists" id="item">
                        ...
                        <php>if($image['tags'] == $item['name']) {echo 'selected';}</php>
                        >{$item['name']}</option>
                    </volist>
                </select>
                    </span></div>
            ...
            <div id="uploader" class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span
                            class="c-red">*</span>简体图片(最大{$Think.config.UPLOAD_IMAGE_MAX_SIZE}M)：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <!--用来存放文件信息-->
                    ...
                    <div class="btns">
                        ...
                        <span class="tags-image-size-span ml-15 c-999">建议尺寸为：200X200，大小不超过20K。</span>
                    </div>
                </div>
            </div>
            ...
            $("#form-advert-add").validate({
                ...
                submitHandler: function (form) {
                    ...
                    if ($('input[name=land_image_id]').val()) {
                        var url = '/1fd3ab97ee/business/image/change';
                    } else {
                        var url = '/1fd3ab97ee/business/image/add';
                    }
