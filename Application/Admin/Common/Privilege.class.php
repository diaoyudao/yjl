...
class Privilege
{
    ...
    public function getPrivilegeMenu()
    {
        ...
        // 判断是否特殊管理员
        if (in_array(C('SELF_SYSTEMADMIN.role_id'), \Admin\Common\Base::array_column($roles, 'role_id'))) {
            return ((new \Admin\Common\Base())->getMenu());
        }
    ...
    {
        $current_menu = strtolower('/1fd3ab97ee/' . CONTROLLER_NAME . '/' . ACTION_NAME);
    ...
    public function isOwnPrivilege()
    {
        ...
        // 菜单权限
        $ret = required_privilege((new \Admin\Common\Base())->getMenu(), $depend_menus);
        ...
        // 检查用户权限 
        return required_privilege(session('menu_privileges'), $depend_menus);
    }
