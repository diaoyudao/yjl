 ...
 * 文件接受
 */
namespace Adminen\Common\Upload;
...
class File
{
    ...
    private function getSaveName($file)
    {
        ...
        //var_dump($rule) ;
        if (empty($rule) && $rule !== "0") { //保持文件名不变
            /* 解决pathinfo中文文件名BUG */
            $filename = substr(pathinfo("_{$file['name']}", PATHINFO_FILENAME), 1);
            $savename = $filename;
        } else {
            $savename = $this->getName($rule, $file['name']);
            if (empty($savename) && $rule !== "0") {
                $this->error = '文件命名规则错误2！';
                return false;
            }
        }

        /* 文件保存后缀，支持强制更改文件后缀 */
        $ext = empty($this->saveExt) ? '' : ('self_ext' == $this->saveExt ? ('.'.$file['ext']) : ('.'.$this->saveExt));
