...
class NoticeController extends Controller
{
    ...
    private function changeSubmit()
    {
        ...
        $data['note'] = (new \Adminen\Common\Textarea($note))->convertEnterToNone();
    ...
    private function changePage()
    {
        ...
        if ($notice['desc']) {
            $notice['desc'] = (new \Adminen\Common\Textarea($notice['desc']))->convertBrToEnter();
        }

        if ($notice['note']) {
            $notice['note'] = (new \Adminen\Common\Textarea($notice['note']))->convertEnterToNone();
        }
    ...
    {
        $source_levels = C('MENU_NOTICE_LEVELS');
        \Think\Log::write(json_encode($source_levels));
        $select_levels = (new \Adminen\Common\MenuPrivilege())->filterAuthorityLevels($source_levels);
        \Think\Log::write(json_encode($select_levels));
        $this->assign('select_levels', $select_levels);
        $this->display();
    }
