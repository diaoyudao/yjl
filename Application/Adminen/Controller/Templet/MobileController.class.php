...
class MobileController extends Controller
{
    public function lists()
    {
        ...
        $source_levels = C('MENU_MOBILE_IMAGE_LEVELS');
        $filter_levels = (new \Adminen\Common\MenuPrivilege())->filterAuthorityLevels($source_levels);
    ...
    private function deleteOne($parameter)
    {
        ...
        $temp_log_string = '删除mobile图片：' . $advert['name'];
    ...
    private function changeSubmit()
    {
        ...
        $temp_log_string = '修改mobile图片：' . $paramter['name'];
        (new StaffLog)->add($temp_log_string);  // 日志记录
    ...
    private function changePage()
    {
        ...
        if ($advert['desc']) {
            $advert['desc'] = (new \Adminen\Common\Textarea($advert['desc']))->convertBrToEnter();
        }
    ...
    private function addSubmit()
    {
        ...
        $temp_log_string = '新增mobile图片：' . $paramter['name'];
        (new StaffLog)->add($temp_log_string);  // 日志记录
    ...
    private function parameterCheck($type = 'add')
    {
        ...
        if ($paramter['desc']) {
            $paramter['desc'] = (new \Adminen\Common\Textarea($paramter['desc']))->convertEnterToBr();
        }
