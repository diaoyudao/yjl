...
{
    const CITY_TAGS = '酒店-城市';
    const TYPE_TAGS = '酒店-属性';
    const LAND_TAGS = 'hotel';
    ...
    public function lists()
    {
        ...
        $types = M('type')->where("tags = '%s' and status=%d", self::TYPE_TAGS, C('STATUS_VALID'))->select();
...
//        $parameter = $this->searchParameterCheck();
...
//        dump($room_images);
    ...
    private function mergeSql($parameter)
    {
        ...
        $sql = "select a.*,(select name from type where type_id=a.city_id) city_name,
                  (select name from type where type_id=a.type_id) type_name 
    ...
    private function queryWhere($paramter)
    {
        ...
        if (!empty($paramter['type_id'])) {
            $sql_where .= " and ( type_id like '%s' )";
            $sql_parameter[] = $paramter['type_id'];
            $where['type_id'] = $paramter['type_id'];
        }
...
//        $status=abs($advert['status']-1);
        ...
        $temp_log_string = '待审核酒店：' . $advert['name'];
        (new StaffLog)->add($temp_log_string);  // 日志记录
...
//        $status=abs($advert['status']-1);
        ...
        $temp_log_string = '发布酒店：' . $advert['name'];
        (new StaffLog)->add($temp_log_string);  // 日志记录
...
//                $advert_status[]= $model->find($id);;
    ...
    private function changeSubmit()
    {
        ...
        if ($data['note']) {
            $data['note'] = (new \Adminen\Common\Textarea($data['note']))->convertEnterToNone();
        }
        $model = M('land');
    ...
    private function changePage()
    {
        ...
        if ($land['desc']) {
            $land['desc'] = (new \Adminen\Common\Textarea($land['desc']))->convertBrToEnter();
        }

        if ($land['note']) {
            $land['note'] = (new \Adminen\Common\Textarea($land['note']))->convertEnterToNone();
        }
    ...
    private function addSubmit()
    {
        ...
        $temp_log_string = '新增酒店：' . $paramter['name'];
        (new StaffLog)->add($temp_log_string);  // 日志记录
