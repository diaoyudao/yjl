    ...
    <div class="section1">
        <div class="content container">
            <p><span><a href="/index">首页</a></span>&nbsp;&nbsp;&nbsp; > &nbsp;&nbsp;
                ...
                <span><if condition="'教育' == $two_path['name']">{$news.name}<else />详情页</if></span>
            </p>
            <h2>{$news.name}</h2>
            <p class="second">发布时间：<span class="time">{$news.begin_time}</span><notempty name="news.source">&nbsp;&nbsp;&nbsp;&nbsp;来源：<span
                    class="source">{$news.source}</span></notempty>&nbsp;&nbsp;&nbsp;&nbsp;阅读数：<span>{$news.view_count}</span>&nbsp;&nbsp;&nbsp;&nbsp;分享到:
