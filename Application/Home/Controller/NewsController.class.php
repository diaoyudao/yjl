...
class NewsController extends Controller
{
    ...
    public function index()
    {
        ...
        $head_news = M('notice')->where($where)->order('weight desc,begin_time desc')->limit(1)->select();
        ...
        // 分类新闻
        $news_app = new \Home\Common\News\Service();
...
//        $this->assign('head_news', $head_news[0]);
    ...
    public function detail()
    {
        ...
        $this->assign('two_bar_codes', \Home\Common\Service::getTwoBarCodes());
...
//        $this->assign('head_news', $head_news);
    ...
    private function getAllLevelByNews($levels)
    {
        foreach ($levels as $level) {
            if ($level['name'] == \Home\Common\News\Service::SHOW_TAGS_LISTS['news']) {
                $news_level = $level;
                break;
            }
        }
    ...
    {
        foreach ($levels as $level) {
            if ($level['name'] == \Home\Common\News\Service::SHOW_TAGS_LISTS['media']) {
                $news_level = $level;
                break;
            }
        }
    ...
    private function getIndexNotice($news_app,$type,$order,$head_news,$common_where)
    {
        ...
        // $sql_paramter['_string'] = "end_time is null or end_time >= '" . (new \DateTime())->format('Y-m-d') . "'";
